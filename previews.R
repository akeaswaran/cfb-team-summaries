...
generate_matchup_df <- function(home_id, away_id, season_override) {
    home_data <- readr::read_csv(paste0("./data/", ifelse(is.null(season_override), current_week$season[1], season_override), "/", home_id, "/overall.csv"), show_col_types = FALSE) |>
        clean_team_data()

    away_data <- readr::read_csv(paste0("./data/", ifelse(is.null(season_override), current_week$season[1], season_override), "/", away_id, "/overall.csv"), show_col_types = FALSE) |>
        clean_team_data()
