...
summarize_passer_df <- function(x) {
    tmp <- x %>%
        ...
        summarize(
            ...
            passer_player_name = dplyr::first(passer_player_name, na_rm = T),
            plays = n(),
...
summarize_receiver_df <- function(x) {
    tmp <- x %>%
        ...
        # ) %>%
        summarize(
            receiver_player_name = dplyr::first(receiver_player_name, na_rm = T),
...
summarize_rusher_df <- function(x) {
    tmp <- x %>%
        ...
        summarize(
            ...
            rusher_player_name = dplyr::first(rusher_player_name, na_rm = T),
            plays = n(),
            games = length(unique(game_id)),
            team_games = dplyr::last(team_games),
            playsgame = plays / games,
...
for (yr in seasons) {
    ...
    plays <- plays %>%
        ...
        mutate(
            ## need these lines for yds_rushed and yds_receiving until cfbfastR is updated with better handling for 2025 pbp
