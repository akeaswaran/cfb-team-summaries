...
write_team_csvs <- function(data, team, yr, type) {
    ...
    readr::write_csv(data, glue("./data/{yr}/{team}/{type}.csv"), na = "NA")
}
...
process_season <- function(yr) {
    ...
    cli::cli_alert_info("Creating folder /data/ if necessary")
    dir.create(file.path("./data", ""), showWarnings = FALSE)

    cli::cli_alert_info("Creating folder /data/{yr} if necessary")
    dir.create(file.path("./data", glue("{yr}")), showWarnings = FALSE)

    cli::cli_alert_info("Writing year CSVs to folder /data/{yr} [{fmt_elapsed(season_start)}]")
    readr::write_csv(team_off_pctls, glue("./data/{yr}/percentiles.csv"), na = "NA")
    readr::write_csv(team_data, glue("./data/{yr}/overall.csv"), na = "NA")
    readr::write_csv(team_qb_data, glue("./data/{yr}/passing.csv"), na = "NA")
    readr::write_csv(team_rb_data, glue("./data/{yr}/rushing.csv"), na = "NA")
    readr::write_csv(team_wr_data, glue("./data/{yr}/receiving.csv"), na = "NA")

    cli::cli_alert_info("Writing team CSVs to folder /data/{yr}")
    team_data |>
        group_by(team_id) |>
        group_walk(~ write_team_csvs(.x, .y$team_id, yr, "overall"), .keep = TRUE)

    cli::cli_alert_info("Writing player CSVs to folder /data/{yr}")
    team_qb_data |>
        group_by(team_id) |>
        group_walk(~ write_team_csvs(.x, .y$team_id, yr, "passing"), .keep = TRUE)

    team_rb_data |>
        group_by(team_id) |>
        group_walk(~ write_team_csvs(.x, .y$team_id, yr, "rushing"), .keep = TRUE)

    team_wr_data |>
        group_by(team_id) |>
        group_walk(~ write_team_csvs(.x, .y$team_id, yr, "receiving"), .keep = TRUE)

    cli::cli_alert_success("Finished {yr}: wrote {nrow(team_data)} teams in {fmt_elapsed(season_start)}")
}

script_start <- Sys.time()
purrr::walk(seasons, process_season)
